...
def print_call_info(func):
    ...
    def wrapper(*args, **kwargs):
        ...
        # Inspect the stack to find the parent function
        stack = inspect.stack()
        if len(stack) > 1:
            parent_function = stack[1].function

        if parent_function == '<module>':
            parent_function = 'None (top level)'

        print(f"Function '{current_function}' called by '{parent_function}'")
...
def debounce(wait):
    def decorator(fn):
        def debounced(*args, **kwargs):
            ...
            debounced._timer = Timer(wait, lambda: fn(*args, **kwargs))
...
class dashboard_object(object):
    ...
    def __init__(self, em_collection, **kwargs):
        ...
        self.width_slider = widgets.IntSlider(min=10, max=150, value=self.width, step=5, description='Width', continuous_update=False)
    ...
    @print_call_info
    def precompute_cache(self, ix_range=None, iy_range=None):
        print("precomputing")
        xlen, ylen, zlen = self.demimage.shape
        ix_range = ix_range or range(xlen-1)
        ...
        self.dem_cache = cache

    @print_call_info
    def load_cache(self):
        try:
            with open(self.cache_file, 'rb') as f:
                self.dem_cache = pickle.load(f)
        except FileNotFoundError:
            self.dem_cache = {}
            self.precompute_cache()

    def get_dem_at(self, ix, iy):
        return self.dem_cache.get((ix, iy), (None, None))
            ...
            if self.mouseover:
                ...
                if event.inaxes == self.ax2:
                    ...
                    if ix >= 0 and ix < xlen and iy >= 0 and iy < ylen:  # Check if ix and iy are within the bounds
                        ...
                        self.demplot_mouseover_vert.set_xdata(the_max_temp)
    ...
    def update_figure(self, rtemp, gtemp, btemp, sigma, algorithm, gfac=1.0/2.2, plt_emmax=3.0e27,
        ...
        if width is not None and not width == self.width:
            self.width = width
