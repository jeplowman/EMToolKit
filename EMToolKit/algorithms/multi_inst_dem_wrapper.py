...
# for that.
def multi_inst_dem_wrapper(datasequence, wrapargs={}):
	nc = len(datasequence)
	nc = len(datasequence)
	...
	# maps = []
	# import sunpy.map
	# for dat in datasequence:
	# 	maps.append(sunpy.map.Map(dat.data, dat.meta))

	# asdf=[print(x) for x in dir(dat) if not "__" in x]

	# Pull out two maps
	# aia_maps = maps[0:-1]
	# xrt_map = maps[-1]

	# Reproject the AIA maps to the XRT map
	# aia_reproj_maps = [mp.reproject_to(xrt_map.wcs) for mp in aia_maps]
	...
	# Scale the uncertainties by the area ratio
	...
	# xrt_reproj_map.plot(axes=ax1, alpha=0.4)
	# plt.title(f'XRT overlaid on AIA\n {xrt_reproj_map.data.shape}')


	# small_maps = [mp.resample(xrt_map.data.shape * u.pix) for mp in maps]

	# for smp in small_maps:
	# 	print(smp.data.shape)
	# 	smp.peek()
